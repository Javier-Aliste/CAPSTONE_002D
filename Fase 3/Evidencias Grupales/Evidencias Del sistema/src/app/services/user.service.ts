...
export class UserService {
  ...
  async getClientesExcluyendo(clienteId: string): Promise<Cliente[]> {
    try {
      ...
      return clientesSnapshot.docs
        ...
        .filter(cliente => clienteId !== cliente.id); 
    } catch (error) {
 ...
 // Método para actualizar el mostrarNombreReal
 async actualizarMostrarNombreReal(clienteId: string, mostrar: boolean): Promise<void> {
  try {
    const clienteRef = doc(this.firestore, `clientes/${clienteId}`);
    await updateDoc(clienteRef, { mostrarNombreReal: mostrar });
    console.log('Campo mostrarNombreReal actualizado exitosamente.');
  } catch (error) {
    console.error('Error al actualizar mostrarNombreReal:', error);
  }
}
}
